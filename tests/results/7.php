...
Parsed::$templates['components/form-group'] = function ($data, $slots) {
    extract($this->data); $_attrs = array_diff_key($this->attrs, array_flip(['this','_slot','label','type','options','name','val','value','error',]));
...
if (empty($this->slots("default"))) { if ($type === 'text') { 
...
<?php ; }  elseif ($type === 'checkbox') {  foreach ($options as $name => $label) { 
; ?><label>
            <input type="checkbox" <?php echo (in_array($name, $values) ? 'checked' : '') ?> value="1">
            <?php echo htmlspecialchars($label); ?>
        </label>
<?php ; }  }  elseif ($type === 'radio') {  foreach ($options as $val => $label) { 
; ?><label>
            <input type="radio" <?php echo ($val == $value ? 'checked' : '') ?> name="<?php echo $name; ?>" value="<?php echo $val; ?>">
            <?php echo htmlspecialchars($label); ?>
        </label>
<?php ; }  }  elseif ($type === 'select') { 
; ?><select class="form-control"><?php ; foreach ($options as $val => $label) {  ?><option <?php echo ($val == $value ? 'checked' : '') ?> value="<?php echo $val; ?>"><?php echo htmlspecialchars($label); ?></option><?php  } ; ?></select>
