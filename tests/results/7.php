...
Parsed::$templates['components/form-group'] = function ($data, $slots) {
    extract($this->data); $_attrs = array_diff_key($this->attrs, array_flip(['class','label','value','placeholder','name','values','val','_attrs','k','v','error',]));
...
<?php }  ?></select>
<?php }   elseif ($type === 'textarea') {  ?><textarea class="form-control" <?php foreach($_attrs as $k=>$v) echo "$k=\"$v\" "; ?> placeholder="<?php echo $placeholder ?? $label; ?>"><?php echo htmlspecialchars($value); ?></textarea>
...
Parsed::$templates['./temp/7'] = function ($data, $slots) {
    extract($this->data); $_attrs = array_diff_key($this->attrs, array_flip([]));
      $this->comp[0] = Parsed::template("components/form-group", ['type' => 'checkbox', 'name' => 'options', 'label' => 'Options', 'options' => ['o1' => 'A', 'o2' => 'B'], 'values' => ['o1']]);  $this->comp[0]->render($this->data); ?>
