...
Parsed::$templates['block_b1_slot_61cc702c06a62'] = function ($data, $slots) {
    extract($data); $_attrs = array_intersect_key($data, array_flip(array_diff($_attrs, [])));
     ?><b11>123</b11><?php 
};
Parsed::$templates['b1?id=61cc702c06a0b'] = function ($data, $slots) {
    extract($data); $_attrs = array_intersect_key($data, array_flip(array_diff($_attrs, ['blocks','slots','slot','a','b','i1','i2','block','data',])));
     ?><?php 
    $blocks = [];
    $blocks[] = Parsed::template('block_b1_slot_61cc702c06a62', array_merge([], []))->setSlots($slots)->setIndex(0);
    if (isset($slots['b1'])) {
...
Parsed::$templates['comp/comp_slot_slot_default_61cc702c0693e'] = function ($data, $slots) {
    extract($data); $_attrs = array_intersect_key($data, array_flip(array_diff($_attrs, ['slots','data',])));
     ?><div>
        <?php Parsed::template('b1?id=61cc702c06a0b', [])->setSlots($slots)->render($data); ?>
    </div><?php 
};
...
$comp1 = $comp0->addSlot('default', Parsed::template('comp/comp_slot_slot_default_61cc702c0693e', []));
