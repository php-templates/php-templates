...
Parsed::$templates['block_b1_slot?id=61d95e3d9eb12'] = function ($data, $slots) {
    extract($data); $_attrs = array_intersect_key($data, array_flip(array_diff($_attrs, ['k',])));
     ?><b11><?php echo htmlspecialchars($k); ?></b11><?php 
};
Parsed::$templates['b1?id=61d95e3d9eaa3'] = function ($data, $slots) {
...
$this->block['b1']->addSlot('b1', Parsed::template('block_b1_slot?id=61d95e3d9eb12', ['k' => $k, '_index' => '0']))->setSlots($slots);
...
Parsed::$templates['./cases/3'] = function ($data, $slots) {
    extract($data); $_attrs = array_intersect_key($data, array_flip(array_diff($_attrs, ['slots','data',])));
     ?><!DOCTYPE html>
<html><body><?php Parsed::template('b1?id=61d95e3d9eaa3', [])->setSlots($slots)->render($data); ?>
